...
/**
 * Direct Client-Side Open-Meteo Fetcher for Static Hosts (GitHub Pages)
 */
async function fetchDirectOpenMeteo(lat: number, lon: number): Promise<WeatherData> {
  ...
  try {
    ...
    console.warn("Direct AQI fetch error:", e);
  }
  ...
  if (daily.time && Array.isArray(daily.time)) {
    for (let i = 0; i < daily.time.length; i++) {
      ...
      dailyForecast.push({
        date: daily.time[i],
        maxTemp: daily.temperature_2m_max ? daily.temperature_2m_max[i] : 25,
        minTemp: daily.temperature_2m_min ? daily.temperature_2m_min[i] : 18,
        weatherCode: code,
  ...
  if (hourly.time && Array.isArray(hourly.time)) {
    ...
    for (let i = 0; i < next24.length; i++) {
      ...
      hourlyForecast.push({
        ...
        humidity: hourly.relative_humidity_2m ? hourly.relative_humidity_2m[i] : 70,
  ...
  const usAqi = aqiCurrent.us_aqi || Math.round((aqiCurrent.pm2_5 || 15) * 4);
  let aqiLabel = "Good";
  ...
  return {
    ...
    current: {
      ...
      feelsLike: current.apparent_temperature ?? 24,
    ...
    aqi: {
      ...
      pm10: aqiCurrent.pm10 || 25,
      no2: aqiCurrent.nitrogen_dioxide || 10,
      so2: aqiCurrent.sulphur_dioxide || 5,
      o3: aqiCurrent.ozone || 30,
      co: aqiCurrent.carbon_monoxide || 300
    },
    daily: dailyForecast,
...
/**
 * Local Severe Alert Generator for Static Hosts
 */
...
export async function fetchForecast(lat: number, lon: number, locationId?: string): Promise<ForecastResponse> {
  ...
  try {
    ...
    // Silent fallback to direct client-side fetching
  }

  // 2. Direct Open-Meteo Fallback (for static hosts like GitHub Pages)
...
export async function sendChatMessage(
  ...
  // Client-side AI Fallback Generator for Static Hosts
